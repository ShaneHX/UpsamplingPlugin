...
__host__ __forceinline__ float linear_upsampling_compute_scale(int input_size, int output_size, bool align_corners)
{
    return align_corners ? float(input_size - 1)/float(output_size - 1) : (float(input_size)/float(output_size));
}

__device__ __forceinline__ float linear_upsampling_compute_source_index(float scale, int dst_index, int intput_size, bool align_corners)  
{
    if(align_corners)
    {
        ...
        T* outputs)
{
    ...
    if(index < n)
    {
        ...
        const float h1r = linear_upsampling_compute_source_index(rate_h, h2, input_h, align_corners);
        const int h1 = int(h1r);
        const int h1p = (h1 < input_h - 1) ? 1 : 0;
        const float h1lambda = h1r - h1;
        const float h0lambda = 1 - h1lambda;

        const float w1r = linear_upsampling_compute_source_index(rate_w, w2, input_w, align_corners);
